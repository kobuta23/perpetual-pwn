...
contract PWN is Ownable {
     ...
     * @return True if successful
     */
    function repayLoan(uint256 _loanId, address _owner) external returns (bool) {
        LOAN.repayLoan(_loanId, _owner);
        ...
        LOAN.closeStream(_loanId,  _owner);
    ...
    function claimLoan(uint256 _loanId) external returns (bool) {
        uint8 status = LOAN.getStatus(_loanId, msg.sender);
